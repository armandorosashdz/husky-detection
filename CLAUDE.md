...
`config.py` has `VALIDATION_DIR = DATA / "validation"` and `MODELS_DIR = RESULTS / "models"` (i.e. `results/models/`) — added ahead of use, **not referenced by any script yet**. Note `MODELS_DIR` points at `results/models/`, which is a *different* location from the actual `models/` folder at repo root where the team's finished `.pt` weights currently live (see gitignore exception below) — if `metrics.py`/`hybrid_inference.py` end up using `MODELS_DIR`, decide whether to reconcile these two "models" locations or keep them intentionally separate (e.g. `models/` = final deliverables, `results/models/` = per-run artifacts).
