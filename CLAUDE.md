...
This is an early-stage academic project (VLM-assisted auto-labeling + YOLO transfer learning pipeline for husky detection). `src/rename_images.py`, `src/vlm_utils.py`, `src/yolo_utils.py`, and `src/auto_labeling.py` (Fase 1, full loop over `data/raw/`) are implemented. `src/train_yolo.py`, `src/hybrid_inference.py`, `src/metrics.py` are still **empty stubs**. `requirements.txt` and `dataset.yaml` are also empty placeholders. Before assuming a function/module exists, check the file is non-empty.
...
- `config.AUTO_LABELING_LIMIT` caps how many images `auto_labeling.py` processes per run (`None` = all of `data/raw/`). Currently set to a small int (`5`) for local testing — `data/labels_auto/` and `data/labels_check/` only have output for that many images so far, not the full 100. Set back to `None` before the real full run.
