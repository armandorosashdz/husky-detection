...
**CUDA memory fragmentation across many `ask()` calls in a loop:** on Kaggle (4B, GPU), `auto_labeling.py` OOM'd on image 27/100 (`torch.OutOfMemoryError`, GPU 0 at ~14.27/14.56GB used) despite each individual call needing far less — each of the 100 images has a different resolution, so each `generate()` call allocates differently-shaped CUDA tensors (input, KV-cache) that PyTorch's caching allocator can't cleanly reuse between calls, so usage creeps up over a long loop instead of staying flat. Fixed in `ask()` by explicitly `del inputs, output_ids` + `torch.cuda.empty_cache()` after decoding the response, before returning (no-ops safely when there's no CUDA). Not yet re-verified on Kaggle against the full 100-image run — confirm this actually clears it up before assuming it's fully fixed.
