...
En Kaggle, `transformers`/`accelerate`/`ultralytics` ya vienen preinstalados pero desactualizados, por lo que `pip install -r requirements.txt` no los actualiza. Es necesario forzar la actualización antes de correr el pipeline:
```bash
!pip install -U transformers accelerate ultralytics -q
```
